...
if (!existsSync(headersPath)) {
  ...
  if (!/img-src[^;]*data:/.test(csp))
    fail('csp-data', 'img-src needs data: blob: for admin');
  if (!/font-src[^;]*data:/.test(csp)) fail('csp-data', 'font-src needs data:');
  if (!/connect-src[^;]*\.tina\.io/.test(csp))
    fail('csp-connect', 'connect-src missing *.tina.io');
  if (!/connect-src[^;]*\.tinajs\.io/.test(csp))
    fail('csp-connect', 'connect-src missing *.tinajs.io');
  if (headers.includes('__UKBT_CSP_SCRIPT_HASHES__'))
    fail('csp-unstamped', 'placeholder not stamped — re-run stamp-csp.mjs');
...
for (const file of globSync('**/*.html', { cwd: distDir })) {
  ...
  if (html.includes('[object Object]'))
    fail(
      'html-serialization',
      `${file} contains [object Object] — rich-text unwrap missing`,
    );
}
