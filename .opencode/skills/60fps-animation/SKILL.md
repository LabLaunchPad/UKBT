...
- Verdict: ADOPT. Fully aligned with contract inv.5 (transform/opacity only). FLIP via the Web Animations API needs no dependency. Flag before site use: the grid-`1fr` accordion and `interpolate-size` patterns animate track/height keywords — run `check:motion` (`layout-animation` rule) before placing them in `apps/web/src` styles; `will-change` just-in-time only.
