...
metadata:
  source: iart-ai/web-animation-skills
  upstream_commit: b6dba3e
  audience: ukbt-motion
---
...
## UKBT overlay (binding — overrides upstream where in conflict)

Source: vendored from `iart-ai/web-animation-skills` @ `b6dba3e` (MIT, (c) 2026 iart.ai; see `THIRD-PARTY-NOTICES.md`). Upstream body above is kept intact for reference; this section binds on UKBT work.
- Tokens first (`contracts/MOTION-CONTRACT.md` inv.1): every duration/easing/distance from `packages/truth/src/tokens/approved/motion.json` (fast 120ms, base 200ms, slow 400ms, signature 1600ms, intro 700ms, slideshow 16000ms; standard/enter/exit/emphasized easings; sm 12px/md 20px). No literal `ms`/`ease` in `apps/web/src` component styles — `scripts/check-motion.mjs` fails the gate.
- Reduced motion is UKBT two-tier: STATE changes (drawer, dropdowns, hovers, presses, swaps) resolve instantly; content ENTRANCES resolve as `ukbt-soft-fade` (opacity only). The global `0.01ms` kill in `apps/web/src/styles/base.css` covers all present and future motion.
- Gate before proposing site motion: `pnpm check:motion` (`MOTION_STATUS = PASS`); release needs full `pnpm deploy:verify`.
- Verdict: CONDITIONAL. `backdrop-filter` blur is paint-heavy (see the 60fps skill); any site use requires an opaque `@supports not` fallback, a reduced-transparency solid-panel fallback, contrast defense on text, token durations for hover/enter motion, and `check:perf` PASS with CSS headroom (0.1KB at last merge — treat as BLOCKED until budget is re-approved). Specular sweeps count as ambient loops under the restraint list — entrance-only.
