...
- Verdict: REFERENCE ONLY. `@lottiefiles/*` runtimes are not allowlisted; JSON/canvas players and scroll-driven `setFrame` violate the zero-third-party-JS and budget posture. BLOCKED for site use pending a dependency-allowlist entry + perf-budget re-approval + contract amendment. Keep for export-checklist knowledge (AE/Bodymovin unsupported-feature list) and for declining requests with a CSS/SVG alternative.
