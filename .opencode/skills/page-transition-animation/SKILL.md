...
- Verdict: REFERENCE ONLY. Next.js App Router + `AnimatePresence`/FrozenRouter do not apply (Astro static + `ClientRouter` + View Transitions 350-550ms with the reduced-motion VT kill in `base.css`). Transferable concepts: enter-slower/exit-faster asymmetry, keyed transitions, exit-before-unmount ordering. Never propose `template.tsx`/FrozenRouter code.
