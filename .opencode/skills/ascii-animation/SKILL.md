...
- Verdict: REFERENCE ONLY. Canvas/Three.js generative fields violate the restraint list (no ambient loops). The `scripts/img-to-ascii.mjs` inside this skill is agent-side tooling only — it must never be wired into the site build or shipped to `dist/`. Keep for terminal/demo contexts outside the site.
