...
- Verdict: ADOPT the CSS/SMIL subset (stroke draw-on via the `pathLength="1"` normalization, token-driven keyframes, static reduced-motion end-state). BLOCKED without a contract amendment + dependency-allowlist entry: GSAP MorphSVG/MotionPath, Flubber, offset-path scroll choreography. Inline SVG in the DOM (never `<img>`) so gates can inspect it.
