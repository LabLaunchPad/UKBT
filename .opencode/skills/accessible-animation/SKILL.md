...
- Verdict: ADOPT with tier remap. Upstream Tier 1 (remove vestibular triggers) and Tier 2 (soften to a <=200ms fade) both hold; upstream Tier 3 (keep fades/focus-rings/spinners) holds. Map onto the UKBT two-tier: STATE = instant, ENTRANCE = `ukbt-soft-fade`. The GSAP/Framer/Lenis gating patterns are reference-only (those runtimes are not on the site). SSR/`matchMedia` notes transfer to the Tina island bridge if JS-driven motion is ever added.
