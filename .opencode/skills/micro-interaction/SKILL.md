...
- Verdict: ADOPT the pure-CSS subset (`@starting-style`, `transition-behavior: allow-discrete`, popover/dialog exit). Remap durations into tokens: the 100-250ms band is fast 120/base 200; anything above base needs justification. BLOCKED: Framer `layout`/`layoutId` magic-move, springs/overshoot (violates the no-springs restraint), `AnimatePresence` (no React on site). Press feedback `scale(.98)` already matches the contract matrix.
