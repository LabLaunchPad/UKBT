...
## AMENDMENT 2026-09-26 — fifth official (Faruk Ahmed, Assistant Coach)

**Status:** PROPOSED — needs owner re-approval at PR review (frozen text + amendments above preserved verbatim).

**Authority:** `EV-20260926-003` (owner instruction 2026-09-26: add Faruk Ahmed as Assistant Coach to both officials lists; Sweden-based; monogram until a portrait is supplied; Javed Butt photo unchanged).

**What changed vs the counts cited in the 2026-09-25 amendment:** officials are 5 per surface, not 4 — `players-data.ts` `rawOfficials` + `franchises-data.ts` `uppsalaOfficials` each carry Shaftab Khalid, AGM Sabbir, MD Ashraful Alam, Javed Butt, and Faruk Ahmed. Pictured counts are unchanged (50 players + 4 officials on /players; Uppsala 20+4); Faruk renders the existing monogram variant on both surfaces (`SquadCard` photo-less path; `RawOfficial.photoSlug` now optional, photo/photoAlt conditional). No frozen rule is altered — this records the instance-data count the amendment's arithmetic cites.
